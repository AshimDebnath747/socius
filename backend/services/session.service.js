...
import { registerSessionEvents } from "../sockets/session.socket.js";
import { getIO } from "../sockets/index.js";
export const helpRequestAccept = async (helpRequestId, helperId, mode) => {
    const io = getIO()
    const query = "SELECT * FROM helprequest WHERE id = $1 AND req_status = 'open' ";
    ...
    const query1 =
        "INSERT INTO session (help_request_id, requester_id, helper_id , mode , start_time ) VALUES ($1, $2, $3 , $4 , $5) RETURNING *;";
    ...
    const query2 =
        "UPDATE helprequest SET req_status = 'accepted' WHERE id = $1 RETURNING *";
...
export const endSessionById = async (id) => {
    ...
    try {
        ...
        const { rows } = await client.query(
            "SELECT id, status, help_request_id FROM session WHERE id = $1 FOR UPDATE",
            [id],
        );
        ...
        await client.query(
            `UPDATE session SET status = 'completed', end_time = CURRENT_TIMESTAMP WHERE id = $1`,
            [id],
        );

        await client.query(
            `UPDATE helprequest SET status = 'closed' WHERE id = $1`,
...
export const cancelSessionById = async (id) => {
    ...
    try {
        ...
        const { rows } = await client.query(
            `UPDATE session SET status = 'cancelled', end_time = NOW() WHERE id = $1 AND status = 'active' RETURNING help_request_id`,
            [id],
        );
...
export const getMessages = async (id, userId) => {
    const { rows } = await pool.query(
        "SELECT requester_id , helper_id from session where id=$1",
        [id],
    );
    if (!rows[0]) {
